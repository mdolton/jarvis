...
Note: `asyncio_default_fixture_loop_scope = session` is required for `pytest-asyncio>=1.0` — without it, session-scoped async fixtures (used from Task 4 onward) raise `ScopeMismatch`.
...
```bash
git add pyproject.toml .python-version ruff.toml pytest.ini .gitignore uv.lock jarvis/ tests/
git commit -m "scaffold jarvis Python project with uv, ruff, pytest"
```

Note: `uv.lock` is generated by Step 8 and MUST be committed — this is a service (not a library), reproducible builds depend on it.
